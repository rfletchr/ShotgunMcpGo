...
// docs.json is generated by cmd/fetchdocs (go generate ./...): the whole API
// documentation as a flat map of sections keyed by path ID.
//
//go:embed docs.json
var docsJSON []byte
...
var (
	docsTree *doctree.Tree
	docsOnce sync.Once
	docsErr  error
)

func getDocs() (*doctree.Tree, error) {
	docsOnce.Do(func() { docsTree, docsErr = doctree.Load(docsJSON) })
	return docsTree, docsErr
}
...
const (
	maxRecursiveDocChars = 30000
	maxDocIndexDepth     = 4
)

func handleDocsTopics(args json.RawMessage) (string, bool) {
	var p struct {
		ID    string `json:"id"`
	...
	if p.Depth < 1 || p.Depth > maxDocIndexDepth {
		return fmt.Sprintf("depth must be between 1 and %d", maxDocIndexDepth), true
	}
	if p.ID != "" {
		if _, ok := t.Get(p.ID); !ok {
			return docNotFound(t, p.ID), true
		}
	}
	idx := t.Index(p.ID, p.Depth)
	if idx == "" {
		return fmt.Sprintf("%s has no sections. Fetch it with sg_docs.", p.ID), false
	}
	return idx, false
}
...
func handleDocs(args json.RawMessage) (string, bool) {
	...
	if err := json.Unmarshal(args, &p); err != nil || p.ID == "" {
		return "id is required (call sg_docs_topics to browse)", true
	}
	t, err := getDocs()
	if err != nil {
		return err.Error(), true
	...
	out, err := t.Render(p.ID, p.Recursive, maxRecursiveDocChars)
	if err != nil {
		return err.Error(), true
	}
	return out, false
...
// docNotFound lists the children of the closest ID that does exist, so a typo
// costs one short round trip instead of a full re-browse.
func docNotFound(t *doctree.Tree, id string) string {
	near := t.Nearest(id)
	where := "top level"
	if near != "" {
		where = near
	}
	return fmt.Sprintf("no doc with id %q. Closest match is %s; its sections:\n%s", id, where, t.Index(near, 1))
}
...
func buildServer() *minimcp.Dispatcher {
	...
	s.AddTool(minimcp.Tool{
		Name:        "sg_find_csv",
		Description: "Export ALL matching ShotGrid entities to a CSV file and return only {path, host_path, rows, columns, truncated} — no row data enters the conversation. Use this instead of sg_find whenever you need more than a few dozen rows, or want to filter/aggregate/join the data with SQL: load the file with a SQLite tool's import_csv (create=true, sample_rows=-1), then query it. Link fields are flattened: an entity field becomes `field` (name) + `field.id`; a multi_entity field becomes `field` (names joined by ;) + `field.ids`; a `.type`/`.types` column is added when the field can link to several entity types. `id` is always the first column.",
		InputSchema: &minimcp.Schema{
			Type: "object",
			Properties: minimcp.Properties{
				"entity_type": {Type: "string", Description: "Entity type to query, e.g. Shot, Task"},
				"filters":     {Type: "string", Description: `JSON array of [field, operator, value] triplets. Pass "[]" for all records.`},
				"fields":      {Type: "array", Description: "Fields to export (one or more columns each; see tool description)", Items: &minimcp.Schema{Type: "string"}},
				"filename":    {Type: "string", Description: "Plain file name, e.g. shots_ip.csv (.csv is appended if missing). Written to the server's export directory; no directories allowed."},
				"order":       {Type: "string", Description: `JSON array of [field, direction] pairs, e.g. [["created_at","desc"]]`},
				"overwrite":   {Type: "boolean", Description: "Replace the file if it already exists (default false: fails instead)."},
				"max_rows":    {Type: "number", Description: "Row cap (default 100000). truncated=true in the result means more rows matched."},
			},
			Required: []string{"entity_type", "filters", "fields", "filename"},
		},
	}, handleFindCSV)
	...
	s.AddTool(minimcp.Tool{
		Name:        "sg_docs_topics",
		Description: "Browse the ShotGrid API documentation tree. With no id, lists the top-level pages; with an id, lists that section's children. Each line is `<id>  <title>  (<own chars>[, <total> with children])` — use the sizes to decide what to fetch. Pass an id from here to sg_docs.",
		InputSchema: &minimcp.Schema{
			Type: "object",
			Properties: minimcp.Properties{
				"id":    {Type: "string", Description: "Section ID to list the children of, e.g. reference/shotgun_methods. Omit for the top level."},
				"depth": {Type: "number", Description: "Levels to list below id (default 1, max 4)."},
			},
		},
	}, handleDocsTopics)
	...
	s.AddTool(minimcp.Tool{
		Name:        "sg_docs",
		Description: "Return one section of the ShotGrid API documentation by ID (from sg_docs_topics): its own text plus an index of its child sections. Sections are small — drill down (e.g. reference/shotgun_methods/crud_methods/find) rather than fetching a whole page. IDs are lowercase slugs joined by '/'.",
		InputSchema: &minimcp.Schema{
			Type: "object",
			Properties: minimcp.Properties{
				"id":        {Type: "string", Description: "Section ID, e.g. reference/shotgun_methods/crud_methods/find"},
				"recursive": {Type: "boolean", Description: "Also include the text of every descendant section. Refused, with sizes, if the whole subtree exceeds 30000 chars."},
			},
			Required: []string{"id"},
		},
	}, handleDocs)
